    ...
    public void createStreamConsumerGroups() {
        try {
            if (Boolean.FALSE.equals(redisTemplate.hasKey(CouponStreamConstants.STREAM_KEY))) {
                ...
                log.info("?§Ìä∏Î¶??ùÏÑ±: '{}'", CouponStreamConstants.STREAM_KEY);
            }
            ...
            if (Boolean.FALSE.equals(redisTemplate.hasKey(CouponStreamConstants.DLQ_STREAM_KEY))) {
                ...
                log.info("DLQ ?§Ìä∏Î¶??ùÏÑ±: '{}'", CouponStreamConstants.DLQ_STREAM_KEY);
            }
            ...
            for (String group : groups) {
                try {
                    ...
                    log.info("Ïª®ÏäàÎ®?Í∑∏Î£π ?ùÏÑ±: '{}'", group);
                } catch (RedisSystemException e) {
                    if (e.getCause() instanceof RedisBusyException) {
                        log.info("Ïª®ÏäàÎ®?Í∑∏Î£π ?¥Î? Ï°¥Ïû¨: '{}'", group);
                    } else {
                        log.error("Ïª®ÏäàÎ®?Í∑∏Î£π ?ùÏÑ± ?§Ìå®: '{}'", group, e);
                    }
                }
            }
        } catch (Exception e) {
            log.error("?§Ìä∏Î¶?Ï¥àÍ∏∞??Ï§??§Î•ò Î∞úÏÉù", e);
        }
    }
}
